...
import it.lispa.bdl.server.utils.ExcelMonitoraggioCritici;
 ...
 */
public class MonitoraggioCriticiServer extends MonitoraggioServer {

	/** La Costante serialVersionUID. */
	private static final long serialVersionUID = 8556408473612882094L;

	/** log. */
	private static Logger log = Logger.getLogger(MonitoraggioCriticiServer.class);
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<VOggettoDTO> items = vOggetti.findOggettiByEntiAbilitati(null, null, filtriFlag, filterIstituto, filterProgetto, filterCollezione, cdEntiAbilitati, flAmministratore );

		ExcelMonitoraggioCritici myExcel;
		try {
			myExcel = new ExcelMonitoraggioCritici(items);
			response.setContentType(ExcelWriterUtils.xlsxMimeType);
			response.setHeader("Content-Disposition", "attachment; filename=monitoraggioCritici.xlsx");
